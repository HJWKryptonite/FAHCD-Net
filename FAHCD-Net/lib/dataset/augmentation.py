...
class Augmentation:
    def __init__(
            self, is_train=True, aug_prob=1.0, image_size=256, crop_op=True,
            std_lmk_5pts=None, target_face_scale=1.0, flip_rate=0.5,
            flip_mapping=None, random_shift_sigma=0.05,
            random_rot_sigma=math.pi / 180 * 18, random_scale_sigma=0.1,
            random_gray_rate=0.2, random_occ_rate=0.4,
            random_blur_rate=0.3, random_gamma_rate=0.2,
    ...
    def process(self, img, lmk, lmk_5pts=None, scale=1.0, center_w=0, center_h=0):
        if self.is_train and random.random() < self.aug_prob:
            # 翻转
            img, lmk, lmk_5pts, center_w, center_h = self._flip.process(
                img, lmk, lmk_5pts, center_w, center_h
            )

            # 做随机shape、shift、rotate、scale、align等操作的矩阵
            matrix_geoaug = self._randomGeometryMatrix.process()

            # 选择是否需要裁剪的伸缩和旋转的矩阵 matrix_pre
            if self.crop_op:
...
class RandomTexture:
    def __init__(
            self, random_gray_rate=0.0,
            random_occ_rate=0.0,
            random_blur_rate=0.0,
            random_gamma_rate=0.0,
...
class Flip:
    ...
    def __init__(self, flip_mapping, random_rate):
        self.flip_mapping = flip_mapping  # 关键点对应列表
        self.random_rate = random_rate  # 概率
    ...
    def process(self, image, landmarks, landmarks_5pts, center_w, center_h):
        ...
        # 横坐标偏移
        landmarks_flip[:, 0] = w + flip_offset - landmarks_flip[:, 0]  # 第一列
